...
def solve_ksys(m, hb, verbose=True, opt=False, log_loss=False):
    '''Function to solve the mth k-system using gradient descent optimization on the initally guesses to the fsolve function.
    --
    Params: 
        m (int): index of k-system to solve
        hb (HyperBell): HyperBell object
    ...
    # log individual contributions to loss function
    l0_ls = [] # list of l0 losses
    l1_ls = [] # list of l1 losses
    l2_ls = [] # list of l2 losses
    ...
    def loss(x0, x_ls = None): 
        ...
        l0 = np.sqrt(abs(1-hb.get_norm(x0)))
    ...
    def get_soln(x0, x_ls, log_loss=False):
        ...
        def try_solve(x0):
            ...
            if log_loss:
                try:
                    ...
                    plt.plot(l0_ls, label='l0')
                    plt.plot(l1_ls, label='l1')
                    plt.plot(l2_ls, label='l2')
                    plt.legend()
                    plt.title('Loss function for d = ' + str(hb.d) + ', m = ' + str(hb.m))
                    plt.savefig('loss_plots/loss_d' + str(hb.d) + '_m' + str(hb.m) + '.pdf')
                    plt.show()
                except:
                    fig, ax = plt.subplots(1, 2, figsize=(10,5))
                    ax[0].plot(l0_ls, label='l0')
                    ax[0].plot(l1_ls, label='l1')
                    ax[1].plot(l2_ls, label='l2')
                    ax[0].legend()
                    ax[1].legend()
                    plt.suptitle('Loss function for d = ' + str(hb.d) + ', m = ' + str(hb.m))
                    plt.savefig('loss_plots/loss_d' + str(hb.d) + '_m' + str(hb.m) + '.pdf')
                    plt.show()
